...
class EventDetailPage extends Component {
  ...
  renderGoingButton = () => {
    ...
    if (this.props.user) {
      return <Button secondary="secondary" onClick={this.handleGoing}>{going ? "No :(" : "Yes!"}</Button>
    } else {
  ...
  render() {
    ...
    return (<Fragment>
      <div className="event-detail-page">
        ...
        {this.props.user ? (<Button secondary="secondary" onClick={this.handleChat}>{this.state.chat ? "Close" : "Open"}</Button>) : <Popup trigger={<Button secondary > Open</Button>} content="Please login to join chat room"/>}
...
const mapStateToProps = state => ({user: state.userReducer.user, event: state.eventReducer.currentEvent})

export default connect(mapStateToProps, {setCurrentEvent, addUserEvent, deleteUserEvent})(EventDetailPage)
